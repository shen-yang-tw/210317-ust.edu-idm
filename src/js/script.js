...
/* https://jsfiddle.net/shen_yang_work/19w625m8/ */
const _els = (el) => document.querySelectorAll(el);
...
const clickAccordionToggleChildrenClasses = (clickEl, selfToggleClassArray, accordionEl, clickParent, toggleChild, childToggleClassArray) => {
  _els(clickEl).forEach(item => {
    item.addEventListener('click', event => {
      if (clickParent != '') {
        item.closest(clickParent).querySelectorAll(toggleChild).forEach(item => {
          toggleClasses(item, childToggleClassArray)
        })
        item.closest(clickParent).querySelectorAll(accordionEl).forEach(item => {
...
// clickAccordionToggleChildrenClasses('.list-group-item-action', '', '', '', '.fas', 'hidden');
// clickAccordionToggleChildrenClasses('.list-group-item-action', ['active'], 'ul.toggle', 'li.list-group-item', '.toggle', ['hidden', 'active']);
// clickAccordionToggleChildrenClasses('.list_accordion .arrow', '', 'ul.toggle', 'li.py-1', '.toggle', ['tw-hidden']);

// const _els = (el) => document.querySelectorAll(el)
// const toggleClasses = (el, ...cls) => cls.map(cl => el.classList.toggle(cl)) // '...cls': Rest parameter must be last formal parameter
// const clickToggleChildrenClasses = (clickEl, clickParent, toggleChild, ...cls) => {
//   _els(clickEl).forEach(item => {
//     item.addEventListener('click', event => {
//       if (clickParent != '') {
//         item.closest(clickParent).querySelectorAll(toggleChild).forEach(item => {
...
//   })
// }
// clickToggleChildrenClasses('.list-group-item', '', '.fas', 'fa-angle-right', 'fa-angle-down');
// clickToggleChildrenClasses('.list_accordion .arrow', 'li.py-1', '.toggle', 'tw-hidden');

const setMaxViewHeight = (el) => {
  _els(el).forEach(item => {
...
const eventMaxViewHeight = (event_el, event, height_el) => {
  _els(event_el).forEach(item => {
    item.addEventListener(event, () => {
      _els(height_el).forEach(item => {
        item.style.maxHeight = window.innerHeight - item.getBoundingClientRect().top + 'px'
      })
    })
  })
...
const scrollToTop = (amount) => {
  window.scrollTo({ // If the trigger is a #hash link: <a href="#">, then it's not working
    top: amount,
    behavior: 'smooth'
  })
}
